...
function initLiveTracking() {
    // Center on Coimbatore by default
    const map = L.map('trackingMap').setView([11.0168, 76.9558], 13);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
    }).addTo(map);
    // Create a live marker with custom icon
    const customIcon = L.divIcon({
      className: 'live-marker',
      html: '<div style="background-color: #28a745; width: 20px; height: 20px; border-radius: 50%; border: 3px solid white; box-shadow: 0 0 10px rgba(40, 167, 69, 0.5);"></div>',
      iconSize: [20, 20],
      iconAnchor: [10, 10]
    });
    let liveMarker = L.marker([11.0168, 76.9558], { icon: customIcon }).addTo(map)
      .bindPopup('Live Delivery Vehicle')
      .openPopup();
    // Add start and end markers (optional, can be customized)
    L.marker([11.0168, 76.9558]).addTo(map)
      .bindPopup('Supplier Location');
    // Example destination marker (can be customized)
    // L.marker([11.0183, 76.9725]).addTo(map).bindPopup('Your Location');
    ...
    function connectWebSocket() {
      ...
      ws.onmessage = async function(event) {
        ...
        console.log('Updating live marker to:', lat, lng);
        liveMarker.setLatLng([lat, lng]);
        map.panTo([lat, lng]);
        const place = await getPlaceName(lat, lng);
        console.log('Place name:', place);
        liveMarker.setPopupContent(`Live Delivery Vehicle<br>Lat: ${lat.toFixed(6)}<br>Lng: ${lng.toFixed(6)}<br><b>Place:</b> ${place}`);
    ...
    // Refresh map button (optional, just recenters)
